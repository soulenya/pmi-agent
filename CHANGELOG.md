...
### v4.1.0 — 2026-09-03
**Projects gain a timeline and a whiteboard**
...
- Asked "can Gerry create slides?", the honest answer was no: she could read a deck's text and nothing else. She now builds one. Ask for a pitch deck, product briefing, board update or demo and she produces a real `.pptx`, then imports it to Drive as native **Google Slides**. Creation needs no new Google permission — the existing `drive.file` scope covers importing a file the app made.
- **The theme was measured, not approximated.** Every colour, font, type size and grid position was read off the company's own deck: canvas `12192000×6858000` (Google's exact 16:9, not python-pptx's rounded 13.333in), 18 type sizes down to the odd-looking 7.94pt eyebrow, the 0.56in margin, the three- and four-across card columns. Generated slides sit on the same grid as hand-built ones.
- **Fourteen layouts, named for structure rather than purpose** — `cover`, `section_break`, `statement_media`, `bullets`, `points_with_metrics`, `numbered_cards`, `profile_cards`, `tier_cards`, `metric_cards`, `media_feature`, `split_detail`, `comparison_grid`, `milestone_track`, `hero_number`. The example deck was a fundraise; naming the layouts after fundraising slides would have made them useless for a product briefing.
- **Classification is asked, not assumed.** Four levels — open, confidential internal, confidential proprietary information, confidential trade secret — stamped on every slide. The builder *refuses* to produce a deck without one, so there is no default to get silently wrong. **Open prints no mark at all**, because a deck for an outside audience shouldn't arrive stamped confidential; trade secret prints in the accent red so the top tier reads as a warning rather than as furniture.
- **The theme lives on Drive**, on the same construct as document templates: a doc named **Deck Theme** in the shared PMI Templates folder overrides colours, fonts, sizes, grid and the classification wording, cached locally so decks still build offline and read only when a deck is actually being made. Values are range-checked — a colour that isn't hex, a 9000pt heading, a `cols_3` with four numbers — and a bad line falls back to the built-in value **and is reported in Gerry's reply**. Levels can be relabelled but not added or removed, and `open` cannot be given a label.
- **Text overlap, root cause.** The first build overlapped badly in two places at once. Two causes: every box had a hardcoded height, so multi-line text ran into whatever sat at the next fixed offset; and `line_spacing` as a float writes OOXML `spcPct`, which **PowerPoint measures against the single-line box but Google Slides measures against the raw font size** — a 40.42pt headline at 105% got a 42.4pt line box for glyphs needing 48.5pt, collapsing the lines together *and* pushing the first line's ascender up into the eyebrow above. Both reported symptoms, one cause. Spacing is now written as absolute `spcPts`, and the layout engine measures every string, flows from a cursor, sizes card rows to their tallest member and shrinks type that would overrun. Verified: 14 slides, zero overlaps, zero overruns on a deliberately long-string stress deck.
- Reading and editing existing decks is also new — `read_deck`, plus Slides support in the per-file edit permission (replace text across a deck, rewrite one text box, delete a slide). This *does* need Google reconnected once, since it adds the `presentations` scope. Building a deck does not.
- **Routing bug caught before shipping:** the executor prompt routed "formatted deliverable" to `create_docx`, which would have quietly answered every deck request with a Word document. Fixed in the executor and both affected agent prompts.
