...
- **Spelling:** misspelled words are underlined in every text box in the Windows app; right-click one for suggestions, or **Add to dictionary**. Right-clicking selected text offers Cut, Copy and Paste.
- The left panel is grouped: **Projects & rooms**, **Conversations**, then **Asked about something**, **Voice sessions** and **Routines** folded up with a count. Hub conversations you have not opened on this computer yet sit under **On the hub**; once opened they join the list above. **Gerry** on the left rail opens this page; the side panel's dropdown lists the same conversations (hub ones under *On the hub*), and its open-page button jumps to the conversation here.
- **Pick up where you left off, on any device.** While this computer is connected to the hub, every conversation you have with Gerry here is also kept on the hub, under the same name — and everything you say to Gerry on the hub (a phone, a browser) comes back here. Open a conversation on either side and it is brought up to date first; after every answer the new messages go across. Existing conversations are carried up once, shortly after the app starts. Attachments and routine (scheduled-task) conversations stay on this computer.
...
**Ask her for sub-tasks under a task you already have** — "break *Site preparation* into steps" — and they go under that task. If two tasks answer to the name you used, she asks which one you meant and writes nothing until you say. If none do, she creates that parent and tells you she did. **Existing tasks can be moved too** — "put the four head tasks under *IQT Projects*" — in place, keeping each task's id and history; "make *X* a top-level task again" undoes it. She refuses a move that would put a task under one of its own sub-tasks.
...
**Make the board behave your way.** The gear at the end of the canvas toolbar opens the board's settings: switch **folding off** altogether, choose the **zoom at which folding starts** and **how much earlier each deeper level folds**, set the **mouse-wheel zoom speed** and how much it is **smoothed** (or none), and set the **default text size on task and reference cards**. Pick a card and the **Text** row of the style panel sizes just that one, the same way it does for notes and shapes. These settings are remembered on this computer; **Reset** puts the defaults back.
...
**Joining two tasks files one under the other.** That same line also makes the second task a sub-task of the first, so the Tasks tab, the timeline and the folding all agree with what you drew. **Or just drop one card on top of another** and the line is drawn for you — land the middle of the card you are dragging inside the card you are dropping it on. Little Gerry will not let you make a task the parent of something it already sits under. Remove the line and the task goes back to whichever other line still points at it, so Ctrl+Z puts the board back exactly as it was.
...
- **One header.** The phone's own header names the page, so pages do not repeat their title and explanation underneath it; card padding and headings are tighter; the graph view of Projects, the Kanban board and the project statistics cards are desktop views and do not appear. The New button sits on the first row beside the section switch.
- **Today on a phone** is a short list: a **Waiting for you** line with the counts (tap it for the list), **Overdue**, **Today** (calendar, tasks due, meetings), **Due this week**, the briefing folded under a heading you tap to read, and **Ask Gerry**.
...
- **Thank-you email afterward** — drafted from real meeting topics to the other party, landing in Email Drafts for your review. Ticking this option lists the exact **outside attendees** it would be addressed to (from the calendar invite); colleagues on any company domain are CC'd, never addressed. If the invite has no outside attendees — or names the wrong people — type the addresses into the box on the consent card (Enter, comma or space after each); what you type becomes the To:. Leave it empty and the draft is created with an empty To: for you to fill in. The panel announces the drafting the moment the call ends, and the finished draft rings the notification bell with a link
...
- **Plan before there is money — the Estimate**. Every budget has an **Estimate** section above the ledger for the anticipated cost of a government contract, a proposal or a future R&D effort. It is its own tab in the budget's Google Sheet, kept apart from the ledger so a plan is never mistaken for spend; type lines there or add them here, and both stay in step. Each line has an optional **phase** (Base, Option 1, Year 2, CLIN 0001…), a **kind** (Labor, Materials, Travel, Subcontract, Other), a **category** from the budget's own list, a description, and either **Qty × Unit cost** (hours × rate) or a flat amount. Lines group by phase with a subtotal each.
- **The Estimate card.** The summary row starts with an **Estimate** card: the estimate total, and once an allotment is set, how far under or over it the plan is. Estimated figures never enter **Allotment** or **Remaining** — those are for funds actually issued — so a pro forma can be built out in full without moving what you have to spend.
- **Categories are a dropdown.** In the ledger's add and edit rows and in the estimate's rows, pick a category from the budget's list or choose **＋ New category…** to add one on the spot; it is written to the sheet's Categories tab and selected. **+ Category** beside the category chips does the same without adding a line.
- **On a project's Budget tab** the owner has the same **Let Gerry manage entries** switch, the figures re-read the sheet every 30 seconds (and when you come back to the window), and every project tab refetches when you open it. On the canvas, **double-click a budget card** to jump to the Budget tab.
- **Two ways to total an estimate.** **Simple** adds the lines up. **Cost build-up** prices them the way a government cost volume does: labor, then **Fringe %** on labor, then **Overhead %** on labor plus fringe, then the other direct costs, then **G&A %** on all of it — that is the cost — then **Contingency %** as a reserve on the cost, then **Fee %** on cost plus contingency. Switch modes and set the rates in the Estimate section; they are written to the sheet's Settings tab beside the allotment, so the sheet and the app always agree. Type a rate as 30 or 30%; leave Contingency blank for none.
- **When the contract is awarded, commit the estimate.** One button writes every line to the ledger as an **Allocated** row (in Cost build-up mode the fringe, overhead, G&A and fee pools become a row each), and by default sets the allotment to the estimate total — you are asked first, and you can commit without touching the allotment. The estimate stays where it is as the baseline to compare actuals against; the section shows the date it was committed.
- **Estimates with Gerry**: "add 200 hours of mechanical engineering at $95 to the Phase 2 estimate", "what does the SBIR estimate come to?", "switch the estimate to cost build-up with 30% fringe and 8% fee", "the contract came through — commit the estimate". Reading is always allowed; adding, changing and committing use the same **Let Gerry manage entries** toggle as the ledger, and changes and commits need your confirmation in chat. Gerry can also **set or clear the allotment** on request ("clear the allotment", "set the allotment to $250,000") — only that field moves, and she confirms the figure with you first.
...
- **When an allocated line is paid, mark it spent**: the tick on any Allocated row, or check several rows and use **Mark N as spent**. The line becomes Spent, dated today; Allocated falls and Spent rises by the same amount, so what is free to promise does not move.
- **What is in the bank**: where Odoo is connected, a fifth card shows the company's live bank and cash balance beside the budget's figures, with the per-account split on hover and a refresh. It is the company's balance, the same on every budget — what there is to pay commitments with, not the budget's share of it.
...
- **Linked folders**: link invoice or receipts folders from Drive to any budget. Gerry scans them read-only — reads each PDF/image/CSV (OCR when needed), extracts vendor, date, and amount, picks a category, and totals what she found. Every document becomes a suggestion you review inside the budget (accept, dismiss, or accept all). Your files are never modified or moved. Open the arrow on a linked folder to see its files one by one — which have been read and what was found on each — and **Read** a single one without waiting for a scan.
- **Add an invoice yourself**: drop PDF, image or CSV files onto the invoices panel, choose them from this computer, or paste a **Google Drive link** (dragging a file out of the Drive tab works too). Each is read for its figures and waits for your accept like everything else. A file from this computer is discarded after reading; a Drive file is left where it is, and if it sits in one of the budget's linked folders it is marked read there so the daily scan does not offer it again.
