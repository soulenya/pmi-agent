...
_TOOL_DOCS = {
    ...
    "extract_document": 'Read a scanned PDF or image (certificate, invoice, form, photo of a document) with VISION — works where normal text extraction returns nothing. Returns transcribed text and, with a schema, structured JSON (null for missing fields, never invented). Source is exactly ONE of: {"drive_file_id": str (id or pasted URL)} | {"generated_filename": str} | {"attachment_name": str (file attached to this conversation)}; optional "schema" (object, desired JSON shape), "instruction" (str), "confirm_restricted" (bool — QMS/draft Drive files only after explicit user confirmation).',
