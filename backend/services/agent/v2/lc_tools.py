...
_TOOL_DOCS = {
    ...
    "read_gmail_message": "Read the full body of a specific Gmail message by ID, plus the list of files attached to it. To read inside an attachment use read_gmail_attachment.",
    "read_gmail_attachment": "Read the contents of a file attached to a Gmail message (Word, PDF, text, CSV, images via vision). Args: message_id, attachment_name (optional when one file), offset (paging).",
    ...
    "extract_document": 'Read a scanned PDF or image (certificate, invoice, form, photo of a document) with VISION — works where normal text extraction returns nothing. Also reads FIGURES (Gantt charts, timelines, diagrams) — pass "instruction" saying what you need from them. Returns transcribed text and, with a schema, structured JSON (null for missing fields, never invented). Source is exactly ONE of: {"drive_file_id": str (id or pasted URL)} | {"generated_filename": str} | {"attachment_name": str (file attached to this conversation)}; optional "schema_name" (str — a SAVED schema: invoice, certificate, purchase_order, dd214… preferred), "schema" (object, custom JSON shape), "instruction" (str), "confirm_restricted" (bool — QMS/draft Drive files only after explicit user confirmation). Long documents return in 30k-character pages: when the reply says the document CONTINUES, call again with {"extraction_id": str, "offset": int} from that reply — it re-reads stored text at no cost — and keep going to the end before answering.',
    ...
    "edit_drive_file": 'Edit a granted Drive file IN PLACE — the change is immediately live in the user\'s real document. Refuses unless request_drive_edit_permission was granted for that exact file. Word/Excel/PowerPoint files cannot be edited: call convert_drive_file first. Read the file first, then prefer a targeted "replace" over "overwrite", which discards the entire existing body. A "replace" changes ONE occurrence and returns the surrounding text as proof it landed — read that before acting again, and never repeat an edit you have not confirmed failed. If "find" matches more than once the edit is REFUSED and every match is listed: either extend "find" until it is unique or pass "occurrence" (1-based) to pick one. "all_occurrences": true changes every match and is only for a genuine global rename, never for filling in a blank. Docs: append|replace|overwrite. Sheets: set_cells|append. Text/markdown files: append|replace|overwrite. Slides: replace|set_shape|add_text_box|add_slide|delete_shape|delete_slide — for add_text_box put the SLIDE id in cell_range and pass a "role" (footnote|caption|body|detail|callout|label|figure|heading) plus a "box" {left,top,width,height} in inches; styling comes from the deck theme, so never specify fonts or colours, and read the deck first so the box lands in empty space (an overlapping box is refused). For add_slide pass "slide" (an archetype plus its fields, exactly as create_deck takes — call list_deck_archetypes first) and optionally "position" (1-based, omit to append); the deck\'s theme and classification mark are applied automatically and page numbers are renumbered. Drive version history is the undo path. JSON: {"file_id": str, "mode": str, "text"?: str, "find"?: str, "replace"?: str, "occurrence"?: int, "all_occurrences"?: bool, "cell_range"?: str (A1 for Sheets, object id for Slides), "values"?: [[str]], "role"?: str, "box"?: {"left":float,"top":float,"width":float,"height":float}}.',
    ...
    "convert_drive_file": 'Convert a Word/Excel/PowerPoint/RTF/OpenDocument file on Drive into an editable native Google Doc/Sheet/Slides copy. Call this as soon as edit_drive_file reports a file is not native — never ask the user to convert it themselves or to use "Open with Google Docs". Creates a NEW file with a NEW id and link and leaves the original untouched, so use the returned file_id for every later read and edit, and give the user the new link. Edit permission carries over. JSON: {"file_id": str (ID or pasted URL)}.',
    ...
    "add_budget_entry": 'Append an entry to a budget ledger (written into the Google Sheet, Source=gerry). The status says whether the money has actually moved: "Spent" = out and gone (default), "Allocated" = out, committed but not yet paid, "Collected" = in and received, "Expected" = in, invoiced or forecast but not yet collected. Allocated and Expected are not spend; Allocated still reduces what is left to promise. Amount is always positive. REQUIRES the per-budget \'Let Gerry manage entries\' permission — if off, point the user to the toggle on the Manage Budgets page. JSON: {"description": str, "amount": number, "status"?: "Spent"|"Allocated"|"Collected"|"Expected", "date"?: "YYYY-MM-DD" (default today), "category"?: str, "note"?: str, "budget_title"?: str}.',
    "update_budget_entry": 'Change one existing ledger entry, identified by description (match_amount/match_date disambiguate). Use new_status to move an entry between planned and actual — Allocated to Spent when a committed cost is paid, Expected to Collected when an invoice settles. Requires the per-budget permission PLUS confirm=true after the user explicitly confirmed the exact change. The sheet row is re-read first — never clobbers Sheets-side edits. JSON: {"description": str, "match_amount"?: number, "match_date"?: str, "new_description"?: str, "new_amount"?: number, "new_date"?: str, "new_category"?: str, "new_note"?: str, "new_status"?: "Spent"|"Allocated"|"Collected"|"Expected", "budget_title"?: str, "confirm": true}.',
    ...
    "set_budget_allotment": 'Set or CLEAR a budget\'s allotment (funds actually issued; Remaining is measured against it). Only the Settings tab changes — ledger and estimate untouched. Requires the per-budget permission PLUS confirm=true after the user agreed to the exact figure. JSON: {"amount"?: number, "clear"?: bool, "budget_title"?: str, "confirm": true}.',
    "read_budget_estimate": 'Read a budget\'s cost ESTIMATE — the plan for a government contract or R&D effort before the money exists; kept on its own Estimate tab, apart from the ledger. Lines by phase and kind (Labor/Materials/Travel/Subcontract/Other), the mode (Simple sum or Cost build-up: labor → Fringe % → Overhead % → other direct → G&A % → Fee %), per-phase totals and the estimate total. JSON: {"title"?: str}.',
    "add_estimate_line": 'Add one line to a budget\'s cost estimate (NOT the ledger — no money moves). qty × unit_cost (hours × rate) or a flat amount. category = one of the budget\'s categories (blank falls back to kind on commit). Requires the per-budget \'Let Gerry manage entries\' permission. JSON: {"description": str, "kind"?: "Labor"|"Materials"|"Travel"|"Subcontract"|"Other", "qty"?: number, "unit_cost"?: number, "amount"?: number, "phase"?: str, "category"?: str, "note"?: str, "budget_title"?: str}.',
    "update_estimate_line": 'Change ONE estimate line matched by description (match_phase disambiguates). Requires the per-budget permission PLUS confirm=true after the user agreed to the exact change. JSON: {"description": str, "match_phase"?: str, "new_description"?: str, "new_kind"?: str, "new_qty"?: number, "new_unit_cost"?: number, "new_amount"?: number, "new_phase"?: str, "new_category"?: str, "new_note"?: str, "budget_title"?: str, "confirm": true}.',
    "remove_estimate_line": 'Delete ONE estimate line by description (match_phase disambiguates; ambiguous matches are listed, nothing deleted). Requires the per-budget permission PLUS confirm=true. JSON: {"description": str, "match_phase"?: str, "budget_title"?: str, "confirm": true}.',
    "set_estimate_rates": 'Set how the estimate is totalled: mode "Simple" or "Cost build-up", and Fringe/Overhead/G&A/Contingency/Fee percentages (30 means 30%). Order: labor → fringe → overhead → other direct → G&A = cost; contingency on cost; fee on cost + contingency. Only given fields change. Requires the per-budget permission. JSON: {"mode"?: "Simple"|"Cost build-up", "fringe_pct"?: number, "overhead_pct"?: number, "ga_pct"?: number, "contingency_pct"?: number, "fee_pct"?: number, "budget_title"?: str}.',
    "commit_budget_estimate": 'The contract was AWARDED: write every estimate line (plus the Fringe/Overhead/G&A/Fee pools in Cost build-up mode) to the ledger as Allocated rows and, by default, set the allotment to the estimate total. The estimate stays as the baseline. Refuses if already committed unless force=true. Requires the per-budget permission PLUS confirm=true after the user explicitly agreed. JSON: {"budget_title"?: str, "set_allotment"?: bool (default true), "force"?: bool, "confirm": true}.',
    ...
    "get_calendar_events": "Read the user's Google Calendar for a date range (days_behind, days_ahead). Call it whenever availability matters — what is scheduled, or BEFORE an email or message you draft proposes, confirms or declines a meeting time; offer only free slots. You have calendar access through this tool; never say you cannot see the calendar.",
    ...
    "read_deck": 'Read a Google Slides deck: title, canvas size, slide count, and every shape\'s text with the object id AND its position/size in inches. Read before editing — the positions are how you find empty space for a new text box. Then edit with edit_drive_file mode "set_shape", "add_text_box", "add_slide", "delete_shape" or "delete_slide". JSON: {"file_id"}.',
    "upload_to_drive": (
