...
# Substrings that flag a sender as automated / no-reply. Matched against the
# full From header (display name + address), case-insensitively. Used so
# "Draft today's unread" never replies to machine-generated notes.
_AUTOGEN_SENDER_MARKERS = (
    "noreply", "no-reply", "no_reply", "donotreply", "do-not-reply",
    "do_not_reply", "notifications", "notification@", "notify@", "mailer",
    "mailer-daemon", "automated", "auto-confirm", "autoconfirm", "bot@",
    "@bounce", "bounces@", "postmaster",
    # Common automated/meeting-note sources
    "gemini", "meet-recordings", "calendar-notification", "teams",
    "microsoftteams", "teams.microsoft.com", "@docs.google.com",
    "@resource.calendar.google.com", "atlassian", "jira@", "confluence",
    "slack", "zoom.us", "asana", "trello", "github.com", "gitlab",
    "sentry", "pagerduty", "datadog", "salesforce", "hubspot", "intercom",
    "zendesk", "freshdesk", "surveymonkey", "mailchimp", "sendgrid",
)


def _is_autogenerated_sender(from_header: str) -> bool:
    """True if a From header looks like an automated / no-reply sender.

    Covers no-reply mailboxes plus well-known machine sources (Gemini meeting
    notes, Microsoft Teams, calendar notifications, ticketing/CI bots, etc.)
    so Gerry doesn't draft replies to mail no human will read.
    """
    hay = (from_header or "").lower()
    return any(marker in hay for marker in _AUTOGEN_SENDER_MARKERS)
...
async def gmail_draft_unread_today(
    ...
    for t in threads[:15]:
        tid = t.get("thread_id", "")
        if _is_autogenerated_sender(t.get("from", "")):
            skipped.append({
                "thread_id": tid,
                "subject": t.get("subject", ""),
                "error": "Skipped automated sender (no-reply / notification).",
            })
            continue
        try:
