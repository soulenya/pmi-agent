...
export const FEATURE_GUIDE: Record<string, FeatureGuideEntry> = {
  ...
  projects: {
    ...
    capabilities: [
      ...
      "Shapes hold text \u2014 double-click one and type inside it \u2014 and come as rounded, square, ellipse, diamond or arrow",
      "Select anything on the canvas to change its fill, line, text colour, text size and fade after it is placed, on one item or on a dozen at once",
      "Draw a shape around something already on the board, and an unfilled shape never blocks what sits under it",
      "Notes and text boxes grow as you type instead of giving you a scroll bar; drag the handle to fix a height yourself",
      "Right-click the canvas to place something where you clicked, or a node to edit, duplicate, restack or delete it; alt-click to dig through a stack",
      "A line drawn between two task cards on the canvas becomes a real dependency on the timeline",
      "Create tasks inside a project on its Tasks tab \u2014 title, status, priority, assignee, dates, milestone flag and sub-tasks \u2014 on a hub project as well as a local one",
      "Sub-tasks sit indented under their parent, which folds itself up once you have added one and shows a count when closed",
      "Every row carries its status as a coloured left edge, so the state of the work reads at a glance",
      ...
      "Paste text onto a canvas and it lands as a text box in the middle of what you are looking at",
      "Right-click in the chat panel to copy what you have highlighted, paste into the message box, or send the highlighted text to the open canvas as text, a sticky note or a shape",
      ...
      "Link projects on the Overview: one can wait for another, gate another, run alongside it or be part of it",
      "A gate waits on a named milestone and closes itself when that milestone is finished; it can also be waived",
      ...
      "Create a budget from a project's Budget tab and everyone on the project can read its whole ledger — allocations and expected income included",
      "Set money aside on a project with an Allocated line, so what is committed comes off what is left before it is ever paid",
      "Only the person whose Drive holds the sheet can add or change lines; a pasted Drive link makes a read-only budget you cannot allocate against",
      "Archive a project to hide it without losing anything, and bring it back later; only its owner can delete one for good",
      "Ask Gerry to list your projects or to file a task under one \u2014 she can see shared projects on the hub as well as local ones",
      "Hand Gerry the line items from a contract and ask him to add them all: he writes the batch in one go with dates, priorities, milestones and sub-tasks, and writes nothing at all if any line is wrong",
      "Ask Gerry what is in a project and he reads its whole task list, nested, not just the tasks with his name on them",
      "All of that works on a shared hub project too \u2014 timelines, task lists, scheduling and canvas notes",
      "Chat inside a shared project runs on your own machine, so Gerry still has your Knowledge Base, Google account and files; the hub only keeps the conversation",
      ...
      "A shared project carries its tasks, canvas, timeline and workroom; attached documents stay on the computer they were added to",
      "Ask Gerry to summarise a project, draft its next steps, schedule its tasks or report its status",
  ...
  budgets: {
    ...
    capabilities: [
      ...
      "Mark each line Spent, Allocated, Collected or Expected — so a budget holds what you have promised as well as what you have paid",
      "An Allocated line reserves money that is committed but not yet paid, and comes off what is left straight away",
      "Money coming in is tracked separately and never inflates what you have left to spend",
      "A blank or unrecognised status counts as Spent, so nothing you already have changes meaning",
