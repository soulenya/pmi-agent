...
export function GeneratedFilesPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const [kbTarget, setKbTarget] = useState<{ name: string; displayName: string } | null>(null);
  ...
  const busyName = driveMutation.isPending ? driveMutation.variables?.name : null;

  return (
    <div className={embedded ? "space-y-4" : "max-w-3xl mx-auto space-y-6"}>
      {!embedded && (
        <div>
          <h1 className="text-2xl font-bold">Made by Gerry</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Files Gerry created during conversations and routines. Add one to the Knowledge Base, send it to Drive, or download it.
          </p>
        </div>
      )}
              ...
              <div key={file.name} className="flex items-center gap-3 px-4 py-3">
                ...
                <button
                  onClick={() => {
                    ...
                    setKbTarget({ name: file.name, displayName });
                  }}
                  disabled={busyName === file.name}
                  title="Add to the Knowledge Base (the file is ingested, then removed from this list)"
      ...
      {kbTarget && (
        <SaveToKnowledgeBaseDialog
          subject="this file"
          defaultTitle={kbTarget.displayName.replace(/\.[^.]+$/, "")}
          onSubmit={(meta) =>
            moveGeneratedFileToKB(kbTarget.name, meta.title, false, {
              category_id: meta.category_id,
              is_regulated: meta.is_regulated,
              force: meta.force,
            }).then((res) => {
              qc.invalidateQueries({ queryKey: ["generated-files"] });
              setActionResult({ kind: "ok", text: `“${res.title}” is in the Knowledge Base.` });
            })
          }
