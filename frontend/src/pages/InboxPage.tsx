...
function EmailFrame({ html }: { html: string }) {
  ...
  // Render the email on its own white surface with dark text. HTML email is
  // authored for a light background, so forcing a dark theme leaves messages
  // (especially quoted replies) with black-on-black, unreadable text. A white
  // card guarantees every message renders legibly regardless of dark mode.
  const doc = `<!doctype html><html><head><base target="_blank"><meta charset="utf-8">` +
    ...
    `font-size:14px;line-height:1.5;color:#111827;background:#ffffff;padding:12px;` +
    `word-wrap:break-word;overflow-wrap:break-word;}img{max-width:100%;height:auto;}` +
    `a{color:#1d4ed8;}table{max-width:100%;}blockquote{color:#374151;}</style></head><body>${html}</body></html>`;
  return (
    <div className="rounded-md overflow-hidden bg-white">
      <iframe
        ...
        style={{ width: "100%", height, border: 0, background: "#ffffff" }}
      />
    </div>
  );
}
    ...
    return (
      <div className="rounded border border-zinc-800 overflow-hidden bg-zinc-900">
        ...
        <div className="flex border-t border-zinc-800">
          <button
            onClick={download}
            className="flex-1 text-xs px-2 py-1 text-zinc-400 hover:text-amber-400 flex items-center gap-1.5"
          >
...
const FILTERS: { id: string; label: string; q: string }[] = [
  ...
  { id: "archived", label: "Archived", q: "-in:inbox -in:sent -in:draft -in:trash -in:spam" },
  { id: "spam", label: "Spam", q: "in:spam" },
  { id: "trash", label: "Trash", q: "in:trash" },
  { id: "all", label: "All Mail", q: "in:anywhere" },
];

type SortMode = "newest" | "oldest" | "sender" | "unread";
...
export default function InboxPage() {
  ...
  const sortedThreads = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "sender":
        list.sort((a, b) =>
          senderName(a.from).toLowerCase().localeCompare(senderName(b.from).toLowerCase()),
        );
        break;
      case "unread":
        ...
        <div className="w-80 shrink-0 border-r border-zinc-800 flex flex-col">
          <div className="p-3 space-y-2 border-b border-zinc-800">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <select
                value={!search && !tagFilter ? filterId : ""}
                onChange={(e) => {
              ...
              >
                {(search || tagFilter) && <option value="">Search results</option>}
                {FILTERS.map((f) => (
