...
| **Workbench**              | The shell: a left rail of ten places (Today, Gerry, Projects, Tasks, Team, Mail, Knowledge, Calendar, Compliance, Finance) plus your last five places and a You menu; a search box (Ctrl+K) that jumps to any project, task, document, conversation or person by name, sends a question to Gerry, or runs `/` commands; a peek drawer that opens a task over the page you are on; and Gerry docked on the right, bound to the project you are in |
| **Today**                  | Home. Today's agenda (tasks, meetings, calendar, local and hub), what is overdue, what is waiting for you (approvals, suggestions, notifications), the AI-generated daily briefing, and your recent conversations |
| **Daily Assistant**        | Once-a-day background scan of your Gmail and Google Tasks that surfaces suggested follow-ups and to-dos for human review, **grouped by what they're about** (one box per room or per person you're waiting on) and ordered by how often you accept that kind of suggestion — resolve them individually, by group, or in bulk; the results also appear under Waiting for you |
...
| **Project space**          | Each project has its own screen: overview, timeline, canvas, tasks, budget, pinned material and its conversation with Gerry. Every project gets a conversation with Gerry the moment it is created. Projects carry a goal, a member list with owner/editor/commenter/viewer roles, and a visibility setting — *private* (invisible to everyone else, including the fact that it exists), *shared* with named people, or *company*-wide, which lets **anyone signed in at the firm edit it**, not merely read it |
| **Sharing a project**      | The owner adds people to a project by their work email from the Overview, at *can view*, *can comment* or *can edit*; roles can be changed or revoked at any time. Ownership is not assignable here. Adding the first person opens a private project to *shared*. Someone who has never opened Little Gerry can be added — a dormant account is provisioned and the role is waiting at their first sign-in — but this grants a role, not a way in: identity is still checked at sign-in, and only addresses on the firm's allowed domains can be added, from the same list that governs signing in. A project can never be left without its owner. **New Project** offers the hub as a destination whenever the machine is connected. A shared project carries its tasks, canvas, timeline, links and workroom; document binaries stay encrypted on the install that added them |
| **Project timeline**       | A Gantt chart built from the project's tasks: start and end dates alongside the due date, percent complete, milestones, and finish-to-start dependencies with lag. Drag to reschedule or resize, drag between bars to link. Little Gerry computes earliest and latest dates, slack and the critical path, flags late work, and refuses any link that would make two tasks wait on each other |
| **Project canvas**         | An infinite whiteboard per project: sticky notes, text, shapes, frames, pressure-sensitive freehand ink and encrypted images, alongside live cards for tasks, documents, budgets, bookmarks, files and email threads dragged from the project's pool — which holds every task, budget and pinned item the project has, grouped by kind with a filter and an option to show what is already placed. Cards show live state and open the real record on double-click; a task card's status can be changed from the board. A line drawn between two task cards is a real dependency on the timeline and also nests the second task under the first; dropping one card on top of another draws that line for you, and removing it returns the task to whichever other line still claims it. Zooming out folds the board a layer at a time — loose notes, shapes and links fold into the task card they are joined to (or the nearest one) a stage before sub-tasks fold into their parent, each swallowing card showing a count that opens it again on click. A selection inspector restyles anything after it is placed — fill, line, dashes, text colour and size, opacity — across a whole multi-selection at once. Shapes hold centred text, come in five forms, and can be drawn around existing items; an unfilled shape passes clicks through to what is under it. Notes and text boxes grow to fit their text until you set a height yourself. Undo/redo, align and distribute, snap to objects or grid, explicit z-order with alt-click to dig through a stack, right-click menus, duplicate, copy-paste and keyboard shortcuts. Text pasted onto the board becomes a text box, and text highlighted in the persistent chat panel can be sent to the open canvas as text, a sticky note or a shape from its right-click menu |
| **Project links & gates**  | Projects say how they fit together: one *waits for* another, *gates* another, *runs alongside* it, or *is part of* it. A gate names a milestone in the gating project and closes itself when that milestone is finished, notifying the waiting project's owner; it can also be waived and reinstated. Gates appear on the timeline as a marker on the day they open and flag work scheduled to start ahead of them, without moving any dates. Links that would close a loop are refused and the loop is named. A link also widens what Gerry may read — a linked project's goal, milestones and gate status, and nothing else |
| **Portfolio**              | Every project you can see on one graph, laid out by what waits on what. Each card carries open work, late work, open gates and the next milestone; gates and dependencies are drawn as arrows and parallel work dashed. A link into a project you may not see is drawn without its name — the relationship is not a secret, the name is |
| **Rooms**                  | Persistent co-work spaces for work spanning days or weeks: a goal, a dedicated chat, a progress journal, and pinned artifacts (Drive docs, KB documents, generated files, notes, websites, email threads, tasks, Odoo records, regulatory documents, budgets) that Gerry carries into every message. Every project has one built in, worked on from the project's **Room** tab — pin, unpin, upload a file, write the journal, set the goal, drag a pin onto the canvas — on a hub project as well as a local one, and it is listed for everyone on the project rather than only whoever created it. Rooms that are not a project live under Projects › Rooms. Goal and title edits are journaled with their previous wording so Gerry tracks how the work has turned; files belonging to another room are flagged before she opens them and blocked before she edits them; and she sees when a document was last saved and by whom, so you can both work in it at once. Standing tasks, a morning digest, and Drive sharing so teammates can join their own mirror |
| **One task form**          | Title, project, due, priority and assignee on the face; description, dates, tags and milestone behind More. The same form on the Tasks page, in a project's Tasks tab, in Meetings › Extract Actions and on the Calendar day panel. The chosen project decides whether the task is made locally or on the hub |
| **Voice**                  | A mode of any conversation: the microphone beside Send turns the conversation you are in into a spoken one — listen, answer aloud sentence by sentence, listen again — in the same history. No separate voice session |
| **The hub**                | A shared server at `hub.precisianmedical.com` that is the system of record for a person's work once their machine is signed in to it. Connect once per machine from Settings → The hub and you sign in as yourself, so what you see and what you change is recorded under your name. The Projects screen lists hub projects first, under *On the hub*, read from the server every time — never copied onto your computer; anything still local sits under *Still on this computer* with a one-click move. New projects and project-less tasks go to the hub by default; Gerry's task tools read and write the hub from the desktop. Work created in a shared or company project is held by that project: only someone with editor rights can change it, it cannot be moved elsewhere, and only an owner can release it. When the hub does not answer, an amber bar says so and why — nothing is served from a stale copy. Open the hub's address in a browser while away and you get your projects, budgets, Team, Gerry and Odoo (per-person API key), with a notice that the knowledge base stays on your own computer. Your conversations with Gerry travel: each one the desktop has is carried to the hub under the same id (on open, after every turn, and once for the backlog after start), and what you say to Gerry on the hub comes back to the desktop when the conversation is next opened. On a phone the same address gives a phone layout — bottom tabs Today · Waiting · Team · Tasks · Gerry, one column, list-then-detail — can be added to the home screen, and can push the bell's events (mentions, approvals, due tasks, suggestions) to the device; tablets get the desktop layout |
| **Calendar**               | Scheduled events with meeting integration                                                                                                  |
| **Team**                   | People-to-people chat on the hub: an **Everyone** channel, a channel per hub project (everyone with a role is in it; also a **Team** tab on the project), named groups and direct messages. Markdown, edit/delete your own, @mentions that land under Waiting for you, files up to 15 MB kept on the hub, task and link chips, unread counts on the rail. Polled every few seconds — the hub never pushes to a desktop |
...
| **Budgets**                | Drive-backed budgets kept in a Google Sheet — the sheet is the system of record, editable from either side, with folder scanning, invoice matching, an advisory Odoo cross-check and a per-budget switch for whether Gerry may write. Every line carries a **status**: *Spent* (money gone), *Allocated* (committed, not yet paid), *Collected* (money in) or *Expected* (invoiced, not yet collected), so a budget holds what you have promised as well as what you have paid, and reports what is still **free to allocate**. A blank or unrecognised status counts as *Spent*, so no existing sheet changes meaning. An *Allocated* line is ticked *Spent*, dated that day, once it is paid. Where Odoo is connected the company's live bank balance sits beside every budget's figures. Budgets can reference each other: a sub-budget's spend and its allocations roll up into its parent as two synced lines |
| **Budget estimates**       | The cost plan before the money: the anticipated cost of a government contract, proposal or future R&D effort, on its own **Estimate** tab of the budget's sheet so a plan is never mistaken for spend. Lines have an optional phase (Base, Option 1, CLIN…), a kind (Labor, Materials, Travel, Subcontract, Other) and hours × rate or a flat amount. Totalled **Simple** or as a **Cost build-up** — labor, Fringe %, Overhead %, other direct costs, G&A %, Fee % — with the rates on the sheet's Settings tab. **Commit estimate** on award writes every line to the ledger as *Allocated* and can set the allotment; the estimate stays as the baseline. Gerry reads estimates always and builds them behind the same per-budget write toggle as the ledger |
| **Invoice intake**         | Invoices reach a budget four ways: a watched Drive folder (read-only — files are never modified or moved; its files can also be listed and read one at a time), an on-demand or daily search of the inbox, a file handed over by hand — chosen or dropped onto the panel — which is read for its vendor, date and total and then discarded rather than stored, and a Drive file by pasted link. Everything found waits in an accept-or-dismiss list; **not one figure reaches the sheet without a person saying so**. A duplicate upload is recognised by its contents. A category read off an invoice that the sheet does not yet have is flagged *new* and added to the Categories tab on accept, instead of the line landing uncategorised. Available on the Budgets page and on a project's Budget tab alike — the work runs on the owner's own machine, since reading Drive and Gmail needs their Google account |
| **Budgets on a project**   | A budget attached to a project shows its whole ledger to **everyone on the project**, including allocations and expected income. Only the person whose Drive holds the sheet can create it, attach it, detach it, or add and edit lines — Drive's own permissions decide that, not Little Gerry. A sheet linked from a pasted Drive link is read-only and cannot be allocated against. On a **shared project the sheet is always made on the owner's computer** and the finished figures are copied to the hub, because the hub has no Google account of its own. A budget held only on your own machine can be shared with a project after an explicit warning that its every line becomes readable by the project |
| **Settings**               | Collapsible sections that flag what you haven't set up yet and re-flag themselves when a new release or a new model arrives: LLM model selection, embedding provider, re-index KB, live health monitoring, backup & restore, appearance, notification preferences, and one-click in-app updates |
...
- **Pin to room** — adds the page to a room's pinned items.
...
- **Project space** — Per-project overview, timeline, canvas, tasks, budget, room and chat, with visibility (private / shared / company) and member roles
...
- **Canvas** — Infinite whiteboard with ink, images and live cards for real records; shapes hold text and restyle after placement; task-to-task links are real dependencies and nest one task under the other; zooming out folds the board a layer at a time; the rail is the project's pool of tasks, budgets and pinned material; pasted text lands as a text box
- **Project tasks** — Create and edit tasks inside a project, local or on the hub: status, priority, assignee, dates, milestones, sub-tasks nested under their parent, a status colour on every row and on every canvas card, grouped by status and draggable onto the canvas
...
- **Project room** — Pin, unpin and upload a project's documents, files, pages, notes, tasks and budgets from the project itself, with its journal and goal alongside; works on a hub project, and says plainly when a file's contents can only be opened on the computer that added it
