...
| **Budgets on a project**   | A budget attached to a project shows its whole ledger to **everyone on the project**, including allocations and expected income. Only the person whose Drive holds the sheet can create it, attach it, detach it, or add and edit lines — Drive's own permissions decide that, not Little Gerry. A sheet linked from a pasted Drive link is read-only and cannot be allocated against |
